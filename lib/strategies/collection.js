...
module.exports = function(schema, options) {
  ...
  schema.pre('findOneAndUpdate', function (next) {
        if (!options.suppressVersionIncrement) {
            this._update.__v += 1; // Increment origins version    
        }

        var clone = this._update; //.toObject

        //delete clone._id
        clone.refVersion = this._update.__v;   // Saves current document version
        clone.refId = this._conditions._id;        // Sets origins document id as a reference
